...
var commands = {
					...
					if (/https?:\/\/discord\.gg\/[A-Za-z0-9]+/.test(invite)) {
						...
						bot.joinServer(invite, function(err, server) {
							if (err) {
								...
								bot.sendMessage(msg, "HypeBot.EXE has successfully joined ***" + server.name + "***");
	...
	"about": {
		desc: "About HypeBot.EXE",
		deleteCommand: true, cooldown: 10, usage: "",
		process: function(bot, msg, suffix) {
			bot.sendMessage(msg, "Hi! I'm " + bot.user.username + "! I was created by PrimalMew in service to the Hype Community.");
		}
	},
	"dice": {
		desc: "Rolls a dice.",
