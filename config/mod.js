...
var commands = {
	...
	"stats": {
		...
		process: function(bot, msg, suffix) {
			if (msg.author.id == config.admin_id || msg.channel.isPrivate || msg.channel.permissionsOf(msg.author).hasPermission("manageChannel")) {
				...
				toSend.push("Running HypeBot.EXE v" + version);
	...
	"clean": {
		...
		usage: "<number of HypeBot.EXE messages 1-100>",
			...
			if (suffix && /^\d+$/.test(suffix.split(" ")[0]) && suffix.split(" ")[0].length < 4) {
				if (!msg.channel.isPrivate) {
					if (msg.channel.permissionsOf(msg.author).hasPermission("manageMessages") || msg.author.id == config.admin_id) {
						...
						} else { bot.sendMessage(msg, "⚠ HypeBot.EXE doesn't have the right permissions. You must give it's role the ability to delete messages in this channel. ⚠", function(erro, wMessage) { bot.deleteMessage(wMessage, {"wait": 8000}); }); }
	...
	"kick": {
		...
		process: function(bot, msg, suffix) {
			...
			} else if (!msg.channel.permissionsOf(bot.user).hasPermission("kickMembers")) { bot.sendMessage(msg, "⚠ HypeBot.EXE can't kick members. Give it's role the right permissions to do so. ⚠", (erro, wMessage) => { bot.deleteMessage(wMessage, {"wait": 10000}); });
	...
	"ban": {
		...
		process: function(bot, msg, suffix) {
			...
			} else if (!msg.channel.permissionsOf(bot.user).hasPermission("banMembers")) { bot.sendMessage(msg, "⚠ HypeBot.EXE can't kick members. Give it's role the right permissions to do so. ⚠", (erro, wMessage) => { bot.deleteMessage(wMessage, {"wait": 10000}); });
			} else if (suffix && msg.mentions.length > 0) {
	...
	"jail": {
		...
		process: function(bot, msg, suffix) {
			...
			} else if (!msg.channel.permissionsOf(bot.user).hasPermission("manageRoles")) { bot.sendMessage(msg, "⚠ HypeBot.EXE can't kick members. Give it's role the right permissions to do so. ⚠", (erro, wMessage) => { bot.deleteMessage(wMessage, {"wait": 10000}); });
	...
	"changelog": {
		...
		process: function(bot, msg, suffix) {
			var chanelogChannel = bot.channels.get("id", "148607122010013696");
			if (!chanelogChannel) { bot.sendMessage(msg, "Changelog can only be shown in the Official Blacklight Hype Server.", function(erro, wMessage) { bot.deleteMessage(wMessage, {"wait": 8000}); });
			} else {
	...
	"color": {
		...
		process: function(bot, msg, suffix) {
			if (/^(.*) #?[A-F0-9]{6}$/i.test(suffix)) {
				...
				if (!msg.channel.permissionsOf(bot.user).hasPermission("manageRoles")) { bot.sendMessage(msg, "HypeBot.EXE doesn't have the permissions to edit roles.", function(erro, wMessage) { bot.deleteMessage(wMessage, {"wait": 10000}); }); return; }
	...
	"givecolor": {
		...
		process: function(bot, msg, suffix) {
			...
			if (!msg.channel.permissionsOf(bot.user).hasPermission("manageRoles")) { bot.sendMessage(msg, "HypeBot.EXE doesn't have the permissions to edit roles.", (erro, wMessage) => { bot.deleteMessage(wMessage, {"wait": 10000}); }); return; }
